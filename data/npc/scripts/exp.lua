...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' deseja comprar 1 Item de double exp por 5k de golds?')
			focus = cid
			talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		if doPlayerRemoveItem(cid,2160,5000) then
		doPlayerAddItem(cid, 2008,1)
					focus = 0
					talk_start = 0
				else
					selfSay('Desculpe, voce nao tem 5000 golds.')
				end			
