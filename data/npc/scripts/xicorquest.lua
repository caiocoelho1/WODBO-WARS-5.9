...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' deseja pegar 1 Vocation Xicor por 100 shadow essences? mas lembrese só poderar pegar se  não for reborn')
			focus = cid
			talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		elseif doPlayerRemoveItem(cid,2153,100) then
		doPlayerAddItem(cid, 13487, 1)
					focus = 0
					talk_start = 0
				else
					selfSay('Desculpe, voce nao tem 100 shadow essences.')
				end			
