...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' deseja pegar 1 katana of Sword por 385 Wars Tailons? e essa é a sword free mais forte atualmente')
			focus = cid
			talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		elseif doPlayerRemoveItem(cid,2151,385) then
		doPlayerAddItem(cid, 2400, 1)
					focus = 0
					talk_start = 0
				else
					selfSay('Desculpe, voce nao tem 385 Wars Tailons.')
				end			
