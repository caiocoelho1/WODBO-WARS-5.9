...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' deseja pegar 1 Black Senzu? o preço é 400 wars tailons ')
			focus = cid
			talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		elseif doPlayerRemoveItem(cid,2151,400) then
		doPlayerAddItem(cid, 4852, 1)
					focus = 0
					talk_start = 0
				else
					selfSay('Desculpe, voce nao tem 400 Wars Tailons.')
				end			
