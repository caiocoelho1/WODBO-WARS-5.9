...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
			...
			selfSay('Olá ' .. getCreatureName(cid) .. ' Aqui trocamos seu undeworld helmet por um Arabian Amulet com protect 2% se você ja tem o undeworld helmet ai é só falar yes')
			focus = cid
			talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		elseif doPlayerRemoveItem(cid,2501,1) then
		doPlayerAddItem(cid, 2138, 1)
					focus = 0
					talk_start = 0
				else
					selfSay('Desculpe, voce nao tem Undeworld Helmet.')
				end			
